...
private:
	...
	unsigned const rank_(const SBTElmt*, const unsigned);
	inline void left_rotate(SBTElmt*&);
	inline void right_rotate(SBTElmt*&);
	void maintain(SBTElmt*&, bool);
public:
	SBT();
	void ins(const unsigned);
	unsigned del(const unsigned);
	unsigned const rank(const unsigned);
	unsigned const size();
};
...
unsigned const SBT::rank_(const SBTElmt *root, const unsigned n) {
...
unsigned const SBT::rank(const unsigned n) {
	return rank_(root, n);
}
unsigned const SBT::size() {
	return root->size;
}
